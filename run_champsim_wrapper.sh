...
if [ -z ${RESLT_DIR} ] || [ ! -d ${RESLT_DIR} ] ; then
	mkdir -p ${RESLT_DIR}
fi
...
do
	...
	do
		...
		do
			if [[ ${TRACE} == ${TYPE}* ]] ; then
				if [ ! -d ${RESLT_DIR}/${BINARY}/${TYPE} ] ; then
					mkdir -p ${RESLT_DIR}/${BINARY}/${TYPE}
				fi
				if [ -f ${RESLT_DIR}/${BINARY}/${TYPE}/${BINARY}-${TRACE}-${N_SIM}M.result ] ; then
					...
					bash $PWD/run_champsim.sh ${BINARY} ${N_WARM} ${N_SIM} ${TRACE}.champsimtrace.xz
					mv $PWD/results_${N_SIM}M/${TRACE}.champsimtrace.xz-${BINARY}${OPTION}.txt ${RESLT_DIR}/${BINARY}/${TYPE}/${BINARY}-${TRACE}-${N_SIM}M.result
				fi
			fi
		done
		COUNT=`expr ${COUNT} + 1`
